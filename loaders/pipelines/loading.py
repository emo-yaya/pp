        ...
        if len(results['sweeps']['prev']) == 0:
            for _ in range(self.sweeps_num):
                for j in range(len(cam_types)):
                    ...
                    # ===================== NEW: Both t-1 and t-2 =====================
                    results['lidar2img_t1'].append(np.copy(results['lidar2img_t1'][j]))
                    results['lidar2img_t2'].append(np.copy(results['lidar2img_t2'][j]))
                    # ===================== END NEW =====================
        else:
            if self.test_mode:
            ...
            for idx in sorted(list(choices)):
                ...
                for sensor in cam_types:
                    ...
                    # ===================== NEW: lidar2img based on keyframe t-1 =====================
                    results['lidar2img_t1'].append(compose_lidar2img(
                        results['t1_ego2global_translation'],
                        results['t1_ego2global_rotation'],
                        results['t1_lidar2ego_translation'],
                        results['t1_lidar2ego_rotation'],
                        sweep[sensor]['sensor2global_translation'],
                        sweep[sensor]['sensor2global_rotation'],
                        sweep[sensor]['cam_intrinsic'],
                    ))
                    
                    # ===================== NEW: lidar2img based on keyframe t-2 =====================
                    results['lidar2img_t2'].append(compose_lidar2img(
                        results['t2_ego2global_translation'],
                        results['t2_ego2global_rotation'],
                        results['t2_lidar2ego_translation'],
                        results['t2_lidar2ego_rotation'],
                        sweep[sensor]['sensor2global_translation'],
                        sweep[sensor]['sensor2global_rotation'],
                        sweep[sensor]['cam_intrinsic'],
                    ))
    ...
    def __call__(self, results):
        ...
        if len(results['sweeps']['prev']) == 0:
            for _ in range(self.prev_sweeps_num):
                for j in range(len(cam_types)):
                    ...
                    results_prev['lidar2img_t1'].append(np.copy(results['lidar2img_t1'][j]))
                    results_prev['lidar2img_t2'].append(np.copy(results['lidar2img_t2'][j]))
        else:
            choices = [(k + 1) * interval - 1 for k in range(self.prev_sweeps_num)]
            ...
            for idx in sorted(list(choices)):
                ...
                for sensor in cam_types:
                    ...
                    results_prev['lidar2img_t1'].append(compose_lidar2img(
                        results['t1_ego2global_translation'],
                        results['t1_ego2global_rotation'],
                        results['t1_lidar2ego_translation'],
                        results['t1_lidar2ego_rotation'],
                    ...
                    results_prev['lidar2img_t2'].append(compose_lidar2img(
                        results['t2_ego2global_translation'],
                        results['t2_ego2global_rotation'],
                        results['t2_lidar2ego_translation'],
                        results['t2_lidar2ego_rotation'],
        ...
        if len(results['sweeps']['next']) == 0:
            for _ in range(self.next_sweeps_num):
                for j in range(len(cam_types)):
                    ...
                    results_next['lidar2img_t1'].append(np.copy(results['lidar2img_t1'][j]))
                    results_next['lidar2img_t2'].append(np.copy(results['lidar2img_t2'][j]))
        else:
            choices = [(k + 1) * interval - 1 for k in range(self.next_sweeps_num)]
            ...
            for idx in sorted(list(choices)):
                ...
                for sensor in cam_types:
                    ...
                    results_next['lidar2img_t1'].append(compose_lidar2img(
                        results['t1_ego2global_translation'],
                        results['t1_ego2global_rotation'],
                        results['t1_lidar2ego_translation'],
                        results['t1_lidar2ego_rotation'],
                    ...
                    results_next['lidar2img_t2'].append(compose_lidar2img(
                        results['t2_ego2global_translation'],
                        results['t2_ego2global_rotation'],
                        results['t2_lidar2ego_translation'],
                        results['t2_lidar2ego_rotation'],
        ...
        for i in range(len(results_prev['img']) // 6):
            for j in range(6):
                ...
                results['lidar2img_t1'].append(results_prev['lidar2img_t1'][i * 6 + j])
                results['lidar2img_t2'].append(results_prev['lidar2img_t2'][i * 6 + j])
