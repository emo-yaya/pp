...
class SparseOcc(MVXTwoStageDetector):
    ...
    def _transform_pos_to_current_frame(self, pos_metric, lidar_to_target):
        """仅执行刚性运动补偿，不进行clamp归一化，保持真实物理量纲"""
    ...
    def _process_keyframe(self, img_feats, img_metas, keyframe='t1', prev_pos=None, prev_feat=None, to_frame='t'):
        """提取历史帧高置信度体素，并执行坐标对齐"""
        metas = self._prepare_metas(img_metas, keyframe=keyframe) # _prepare_metas 方法直接从PropBEV拷贝过来

        with torch.no_grad():
            outs = self.pts_bbox_head(img_feats, metas, prev_pos=prev_pos, prev_feat=prev_feat)
            # 获取 Voxel Decoder 最后一层(最高分辨率)的输出
            # occ_preds 结构: (occ_loc, None, seg_pred_2x, query_feat_2x, interval)
            last_occ = outs['occ_preds'][-1]
            occ_loc = last_occ[0]       # [B, K, 3] 此时的interval是1，所以对应真实的 [0~199] 网格索引
            seg_pred = last_occ[2]      # [B, K, CLS] 
            query_feat = last_occ[3]    # [B, K, C]

            # 1. 索引坐标转物理米制坐标
            pc_start = occ_loc.new_tensor(self.pts_bbox_head.pc_range[:3])
            pos_metric = occ_loc * 0.4 + pc_start  # [B, K, 3]
            
            # 2. 运动补偿：对齐到目标帧(当前帧)
            lidar_to_target = self._get_lidar2lidar_transform(img_metas, from_frame=keyframe, to_frame=to_frame)
            pos_aligned = self._transform_pos_to_current_frame(pos_metric, lidar_to_target)

            # 3. 基于语义概率提取 Top-K
            # 计算前景概率 = 1 - free_space 的概率 (假设 -1 是 background/free space)
