...
class SparseBox3DEncoder(nn.Module):
    def __init__(self, embed_dims=[128, 32, 32, 64], vel_dims=2):
        super().__init__()
        self.pos_fc = nn.Sequential(*linear_relu_ln(embed_dims[0], 1, 2, input_dims=3))
        self.size_fc = nn.Sequential(*linear_relu_ln(embed_dims[1], 1, 2, input_dims=3))
        self.yaw_fc = nn.Sequential(*linear_relu_ln(embed_dims[2], 1, 2, input_dims=2))
        self.vel_fc = nn.Sequential(*linear_relu_ln(embed_dims[3], 1, 2, input_dims=vel_dims))

    @torch.no_grad()
    def init_weights(self):
        for fc in [self.pos_fc, self.size_fc, self.yaw_fc, self.vel_fc]:
            for m in fc.modules():
    ...
    def forward(self, box_3d):
        ...
        vel_feat = self.vel_fc(box_3d[..., 8:10])
        return torch.cat([pos_feat, size_feat, yaw_feat, vel_feat], dim=-1)
...
class PropBEVTransformer(BaseModule):
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas):
        cls_scores, bbox_preds, query_feat = self.decoder(query_bbox, query_feat, mlvl_feats, attn_mask, img_metas)
...
class PropBEVTransformerDecoderLayer(BaseModule):
    ...
    def init_weights(self):
        ...
        # Initialize reg_branch
        for m in self.reg_branch.modules():
            if isinstance(m, nn.Linear):
                nn.init.xavier_uniform_(m.weight)
                nn.init.zeros_(m.bias)
        # Last layer: small weights for stable refinement
        nn.init.normal_(self.reg_branch[-1].weight, std=0.01)
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas):
        ...
        # Add anchor embed without yaw before self-attn
        yaw_embed = anchor_embed[..., 128:256].clone()
        anchor_embed_no_yaw = anchor_embed.clone()
        anchor_embed_no_yaw[..., 128:256] = 0.0

        query_feat = query_feat + anchor_embed_no_yaw
        query_feat = self.norm1(self.self_attn(query_bbox, query_feat, attn_mask))

        # Restore yaw component after self-attn
        query_feat[..., 128:256] = query_feat[..., 128:256] + yaw_embed
