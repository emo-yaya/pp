...
class PropBEVHead(DETRHead):
    ...
    def forward(self, mlvl_feats, img_metas, prev_bbox=None, prev_feat=None, use_dn=True):  # NEW: added prev_feat
        ...
        if use_dn:
            ...
            # Skip DN - just create basic query features
            ...
            query_feat = torch.cat([init_query_feat, indicator0], dim=1)
            query_bbox = query_bbox.unsqueeze(0).expand(B, -1, -1)
            query_feat = query_feat.unsqueeze(0).expand(B, -1, -1)
            attn_mask = None
            mask_dict = None
        
        # NEW: Add prev_bbox as propagated queries
        num_prop = 0
        if prev_bbox is not None and prev_bbox.shape[1] > 0 :
            num_prop = prev_bbox.shape[1]
            # NEW: Use prev_feat if provided, otherwise use label embedding
            if prev_feat is not None:
                prop_feat = prev_feat
            else:
                prop_feat = self.label_enc.weight[self.num_classes].unsqueeze(0).expand(B, num_prop, -1)
                prop_feat = torch.cat([prop_feat, torch.zeros(B, num_prop, 1, device=query_feat.device)], dim=-1)
            query_bbox = torch.cat([query_bbox, prev_bbox], dim=1)
            query_feat = torch.cat([query_feat, prop_feat], dim=1)
            
        if num_prop > 0 and attn_mask is not None:
            # Expand attention mask to include propagated queries
            total_size = attn_mask.shape[0] + num_prop
            new_attn_mask = torch.ones([total_size, total_size], device=attn_mask.device) < 0
            
            # Copy original mask (DN and regular query masking)
            old_size = attn_mask.shape[0]
            new_attn_mask[:old_size, :old_size] = attn_mask
            
            # Propagated queries cannot see DN queries (like regular queries)
            dn_pad_size = mask_dict['pad_size'] if mask_dict else 0
            if dn_pad_size > 0:
                new_attn_mask[old_size:, :dn_pad_size] = True  # prop cannot see DN
                new_attn_mask[:dn_pad_size, old_size:] = True  # DN cannot see prop
            attn_mask = new_attn_mask

        img_metas[0]['dn_pad_size'] = mask_dict['pad_size'] if mask_dict is not None else 0
        cls_scores, bbox_preds, query_feat = self.transformer(
            query_bbox,
            query_feat,
            mlvl_feats,
            attn_mask=attn_mask,
        ...
        # NEW: Remove propagated queries from output
        # if num_prop > 0:
            # cls_scores = cls_scores[:, :, :-num_prop, :]
            # bbox_preds = bbox_preds[:, :, :-num_prop, :]
        ...
        if mask_dict is not None and mask_dict['pad_size'] > 0:  # if using query denoising
            ...
            output_query_feat = query_feat[:, mask_dict['pad_size']:, :]  # NEW: exclude dn queries
            ...
            outs = {
                ...
                'all_query_feat': output_query_feat,  # NEW
                'enc_cls_scores': None,
                'enc_bbox_preds': None, 
