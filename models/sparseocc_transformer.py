...
from .propbev_transformer import AdaptiveMixing
...
class SparseOccTransformer(BaseModule):
    ...
    def forward(self, mlvl_feats, img_metas, prev_pos=None, prev_feat=None):
        ...
        ego2lidar = np.asarray([[m['ego2lidar'][0]] for m in img_metas]).astype(np.float32)
        ego2lidar = torch.from_numpy(ego2lidar).to(feat.device)  
        
        img_metas = copy.deepcopy(img_metas)
        img_metas[0]['lidar2img'] = torch.matmul(lidar2img, ego2lidar)

        occ_preds = self.voxel_decoder(mlvl_feats, img_metas=img_metas, prev_pos=prev_pos, prev_feat=prev_feat)
